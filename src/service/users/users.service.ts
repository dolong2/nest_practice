...
export class UsersService {
    ...
    async signin(signinRequestDto: SigninRequestDto): Promise<SigninResponseDto> {
        ...
        const accessOptions = { secret: this.configService.get<string>('ACCESS_SECRET'), expressIn: this.configService.get<string>('JWT_ACESS_EXPIRATION') };
        const accessToken = await this.jwtService.signAsync(accessPayload, accessOptions);

        const refreshPayload = {email: user.email, type: JwtType.REFRESH};
        const refreshOptions = { secret: this.configService.get<string>('REFRESH_SECRET'), expressIn: this.configService.get<string>('JWT_REFRESH_EXPIRATION') };
        const refreshToken = await this.jwtService.signAsync(refreshPayload, refreshOptions);
